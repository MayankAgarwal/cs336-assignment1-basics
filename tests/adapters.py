...
def run_scaled_dot_product_attention(
    ...
    attn = layers.scaled_dot_product_attention(Q=Q, K=K, V=V, mask=mask)
    return attn
...
def run_multihead_self_attention(
    ...
    causal_mha = layers.CausalMultiHeadSelfAttention(
    ...
    causal_mha.load_state_dict(
        {
            ...
            "out_proj": o_proj_weight,
        }
    )

    result = causal_mha(x=in_features)
    return result
...
def run_multihead_self_attention_with_rope(
    ...
    causal_mha = layers.CausalMultiHeadSelfAttention(
        d_model=d_model, num_heads=num_heads, rope_theta=theta, max_seq_len=max_seq_len
    )
    ...
    result = causal_mha(x=in_features, token_positions=token_positions)
    return result
...
def run_rope(
    ...
    rope_layer = layers.RoPE(theta=theta, d_k=d_k, max_seq_len=max_seq_len)

    result = rope_layer(x=in_query_or_key, token_positions=token_positions)
    return result
...
def run_softmax(in_features: Float[Tensor, " ..."], dim: int) -> Float[Tensor, " ..."]:
    ...
    return layers.softmax(x=in_features, dimension=dim)
